...
def gerar_despesas_recorrentes_do_mes(referencia=None):
    """Garante que toda DespesaRecorrente ativa tenha 1 Despesa gerada pro
    mês de `referencia` (hoje por padrão)."""
    referencia = referencia or timezone.now().date()
    ano, mes = referencia.year, referencia.month
    ultimo_dia = calendar.monthrange(ano, mes)[1]

    for recorrente in DespesaRecorrente.objects.filter(ativa=True):
